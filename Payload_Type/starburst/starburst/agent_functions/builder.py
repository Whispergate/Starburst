...
class Starburst(PayloadType):
    ...
    build_parameters = [
        BuildParameter(
            ...
            choices=["bin", "shellcode", "exe", "dll", "service_exe", "elf"],
            default_value="bin",
            description="Output format: bin (raw PIC), shellcode (Crystal Palace), exe/dll/service_exe (Windows PE), elf (Linux static binary)",
        ),
        BuildParameter(
            name="architecture",
            group_name="Main",
    ...
    async def build(self) -> BuildResponse:
        ...
        try:
            ...
            if target_os.lower() == "linux" or output_type == "elf":
    ...
    async def _build_linux(self) -> BuildResponse:
        ...
        try:
            ...
            await SendMythicRPCPayloadUpdatebuildStep(MythicRPCPayloadUpdateBuildStepMessage(
                PayloadUUID=self.uuid,
                StepName="Gathering",
                StepStdout=f"Preparing Linux {arch} agent source",
                StepSuccess=True,
            ))
            ...
            await SendMythicRPCPayloadUpdatebuildStep(MythicRPCPayloadUpdateBuildStepMessage(
                ...
                StepName="Configuring",
                StepStdout=(
                    f"Config: {c2_profile_name} -> {host}:{port}/{post_uri}, "
                    f"sleep={interval}s, jitter={jitter}%, "
                    f"{len(commands)} commands selected"
                ),
                StepSuccess=True,
            ))
            ...
            for sf in linux_src_files:
                if not os.path.exists(os.path.join(linux_src_dir, sf)):
                    resp.status = BuildStatus.Error
                    resp.build_message = f"Linux source not found: {sf}"
                    return resp

            build_dir = tempfile.mkdtemp()
            linux_build_dir = os.path.join(build_dir, "linux")
            shutil.copytree(linux_src_dir, linux_build_dir)
            ...
            src_paths = [os.path.join(linux_build_dir, sf) for sf in linux_src_files]
            ...
            # skip Module Build step for Linux (no PIC module system)
            await SendMythicRPCPayloadUpdatebuildStep(MythicRPCPayloadUpdateBuildStepMessage(
                PayloadUUID=self.uuid,
                StepName="Module Build",
                StepStdout="N/A for Linux builds",
                StepSuccess=True,
            ))
