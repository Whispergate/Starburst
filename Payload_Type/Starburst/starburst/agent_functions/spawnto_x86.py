...
class SpawntoX86Arguments(TaskArguments):
    def __init__(self, command_line, **kwargs):
        ...
        self.args = [
            CommandParameter(
                ...
                default_value="C:\\Windows\\SysWOW64\\rundll32.exe",
            ),
            CommandParameter(
                name="arguments",
                cli_name="Arguments",
    ...
    async def parse_arguments(self):
        if len(self.command_line) == 0:
            raise Exception(
                "spawnto_x86 requires a path to an executable.\n"
                "\tUsage: spawnto_x86 [path] [args]"
            )
...
class SpawntoX86Command(CommandBase):
    cmd = "spawnto_x86"
    needs_admin = False
    help_cmd = "spawnto_x86 [path] [args]"
    description = "Change the default x86 binary used in post-exploitation jobs (shinject, execute_assembly, etc)."
    version = 1
