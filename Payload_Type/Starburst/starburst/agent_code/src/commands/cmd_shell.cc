...
static auto WINAPI declfn shell_thread_fn( LPVOID param ) -> DWORD {
    auto ctx  = static_cast<ShellJobCtx*>( param );
    auto& inst = *ctx->inst;
...
auto declfn starburst::cmd_shell(
    ...
    if ( prefix_len + cmd_len < sizeof(cmdline) - 1 ) {
        memory::copy( cmdline + prefix_len, cmd_str, cmd_len );
    ...
    auto ctx = static_cast<ShellJobCtx*>( inst.heap_alloc( sizeof(ShellJobCtx) ) );
    if ( !ctx ) {
    ...
    HANDLE h_thread = inst.kernel32.CreateThread(
        nullptr, 0,
        reinterpret_cast<LPTHREAD_START_ROUTINE>( shell_thread_fn ),
        ctx, 0, nullptr );
